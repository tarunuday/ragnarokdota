	...
	class connDota
	{
		...
		public function chkSteamID($steamID){
			...
			if(preg_match ($pattern, $steamID )){
			    ...
			    $link = file_get_contents('http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=' . $key . '&steamids=' . $steamID . '&format=json');
			    if ($link !== false) {
				    $myarray = json_decode($link, true);
				    if($myarray!==NULL && isset($myarray['response']['players'][0]['steamid'])){
					die($link);
				    }
			    }
		...
		public function registerTeam(&$ar){
			...
			if ($result) {
				...
				for ($i=1; $i<8 ; $i++) {
					...
					else if(true||($this->chkSteamID($_POST['p_id_'.$i]))){
		...
		public function getTeamList(){
			...
			if ($result && $result->num_rows>0) {
				...
				while ($row1 = $result->fetch_assoc()) {
					$row1['players'] = $this->getPlayersList($row1['id']);
					$row[] = $row1;
				}
				return $row;
			}
		...
		public function getPlayersList($team_id){
			if(!$this->createConn())
				return false;
			$qry = "SELECT * FROM players WHERE team='$team_id'";
			$result = $this->mysqli->query($qry);
			if ($result && $result->num_rows>0) {
				$row = array();
				while ($row1 = $result->fetch_assoc()) {
		...
		public function enableTeam($team_id, $enb){
			if(!$this->createConn())
				return false;
			$qry = "SELECT * FROM teams WHERE id='$team_id'";
			$result = $this->mysqli->query($qry);
			if ($result && $result->num_rows>0) {
				if (($row = $result->fetch_assoc()) && $row['status']==$enb) {
					return true;
				}
				$this->mysqli->autocommit(false);
				$qry = "UPDATE teams SET status='$enb' WHERE id='$team_id'";
				$result = $this->mysqli->query($qry);
				if ($result && $this->mysqli->affected_rows>0) {
					$qry = "UPDATE players SET status='$enb' WHERE team='$team_id'";
					$result = $this->mysqli->query($qry);
					if ($result && $this->mysqli->affected_rows>0) {
						$this->mysqli->commit();
						return true;
